...
- 특히 `492007` / `액티브배당성장70혼합형` fund identity는 확정 출금 주문과 별도로 입금 방향 D+1(예상금액) 주문이 보이는지 반드시 다시 확인하세요.
- `492007` 결과가 확정 출금 주문 1개뿐이면 오답입니다. D+1(예상금액) 입금 cell이 숫자로 보이면 order_type="3", settle_class="1", t_day="02" 주문도 포함해야 합니다.
