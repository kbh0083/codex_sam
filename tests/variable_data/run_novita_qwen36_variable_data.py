...
PROMPT_DIR = ROOT / "tests" / "variable_data" / "prompts_qwen36"
...
def call_novita_json(
    ...
    *,
    ...
    last_parse_info: dict[str, Any] = {"parse_ok": False, "error": "no attempts executed"}
    max_attempts = PARSE_RETRIES + 1
    for attempt in range(1, max_attempts + 1):
        attempt_label = label if max_attempts == 1 else f"{label}_attempt_{attempt}"
        safe_request = {
            "url": endpoint,
            "model": model,
            "message_count": 1,
        ...
        try:
            ...
            elapsed = round(time.perf_counter() - started, 3)
            raise RunBlockedError(f"{attempt_label} timeout after {elapsed}s: {exc}") from exc
        except requests.RequestException as exc:
            elapsed = round(time.perf_counter() - started, 3)
            raise RunBlockedError(f"{attempt_label} request error after {elapsed}s: {exc}") from exc
        ...
        write_json(response_dir / f"{attempt_label}_raw_response.json", raw)
        write_json(response_dir / f"{label}_raw_response.json", raw)

        if response.status_code >= 400:
            raise RunBlockedError(f"{attempt_label} HTTP {response.status_code}: {response.text[:1000]}")
        ...
        if not choices:
            ...
            write_json(response_dir / f"{attempt_label}_message_parts.json", parts)
            write_json(response_dir / f"{label}_message_parts.json", parts)
            all_parts.append(parts)
            last_parse_info = {"parse_ok": False, "error": "choices is empty", "attempts": attempt}
            continue
        ...
        if not isinstance(content_text, str):
            last_parse_info = {"parse_ok": False, "error": f"message.content is not string: {type(content_text).__name__}", "attempts": attempt}
            continue
        parsed, parse_info = parse_json_content(content_text)
        parse_info["attempts"] = attempt
        last_parse_info = parse_info
        if parsed is not None:
            return parsed, parse_info, all_parts

    return None, last_parse_info, all_parts
...
def markdown_link(label: str, target: Any) -> str:
    safe_label = str(label).replace("|", "\\|").replace("]", "\\]")
    target_text = str(target)
    if not target_text:
        return safe_label
    return f"[{safe_label}](<{target_text}>)"
...
def write_report(
    ...
    *,
    ...
    lines = [
        ...
        f"- endpoint: `{endpoint}`",
        f"- cycles: `{cycles}`",
        f"- artifact root: {markdown_link(artifact_root.name, artifact_root)}",
    ...
    for case in cases:
        lines.append(
            f"| `{case['case_id']}` | {case['company']} | {markdown_link(case['source'].name, case['source'])} | "
    ...
    for summary in summaries:
        ...
        lines.append(
            ...
            f"{markdown_link('artifact', artifact)} |"
        )
    lines.extend(
        [
            "",
    ...
    if failures:
        ...
        for summary in failures:
            ...
            if validation_path.is_file():
                lines.append(f"- validation: {markdown_link('validation_summary.json', validation_path)}")
                try:
                ...
                artifact = summary.get("paths", {}).get("execution_dir")
                if artifact:
                    lines.append(f"- artifact: {markdown_link('execution_dir', artifact)}")
            lines.append("")
    lines.extend(
        [
            "## Notes",
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run Novita Qwen3.6 variable-data extraction parameter tests.")
...
def main(argv: list[str] | None = None) -> int:
    ...
    run_id = f"novita_qwen36_variable_data_{now.strftime('%H%M%S')}"
